...
func checkMax(data interface{}, maxStr string) error {
	...
	case int:
		if v > maxLen {
			return fmt.Errorf("value (%v) is higher than maximum value (%v)", v, maxLen)
		}
	case string:
		l := len(v)
		if l > maxLen {
			return fmt.Errorf("length (%v) length is higher than maximim length (%v)", l, maxLen)
		}
	}
	return nil
}
